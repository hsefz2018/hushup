...
inline double calc(double ang)
{
    ...
    double d, l;
    for (int i = 0; i < nc; ++i)
        if (cx[i] * sign(ang) >= 0 && (d = dist_p2l(cx[i], cy[i], ang)) <= cr[i] + EPS) {
...
int main()
{
    ...
    /*while (1) {
        double ang; scanf("%lf", &ang); printf("%.12lf\n", calc(ang / 180 * M_PI));
    }*/

    double tot_area = simpson(-M_PI, M_PI - EPS, EPS);
    /*double tot_area = 0;
    for (double x = -M_PI; x < M_PI; x += M_PI / 10000)
        tot_area += sqr(calc(x)) * M_PI / 20000;*/
    printf("%.12lf\n", tot_area);
